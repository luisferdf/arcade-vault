...
export function FeatureCard({ icon, title, description, color, delayMs }: FeatureCardProps) {
  return (
    <div className={"feature-card " + color} style={delayMs ? { transitionDelay: `${delayMs}ms` } : undefined}>
      <FeatureIcon kind={icon} />
      <div className="ft-title pixel">{title}</div>
      <div className="ft-desc">{description}</div>
    </div>
  );
}
